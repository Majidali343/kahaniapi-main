...
@endif
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="p-6 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <a type="button" class="buttonuser" href="{{ route('package.new') }}">Add Packages</a> 
                                ...
                                @if (isset($packages))
                                    ...
                                    @foreach ($packages as $package)
                                        <tr
                                            ...
                                            <td class="px-6 py-4">
                                                @if($package->Permissions == 1 )
                                                Reading
                                                @elseif($package->Permissions == 2 )
                                                Reading + Listening
                                                @elseif($package->Permissions == 3 )
                                                Reading + Listening + Watching
                                                @elseif($package->Permissions == 4 )
                                                Lifetime
                                                @endif
                                            ...
                                            <td class="px-6 py-4">
                                                <form id="updateStatusForm" data-membership-id="{{ $package->id }}">
                                                    @csrf
                                                ...
                                                <div id="statusMessage-{{ $package->id }}"></div>

                                            </td>

                                            <td class="px-6 py-4">
                                                <a href="{{ route('package.find', $package->id) }}">
                                                    <span class="material-symbols-outlined">edit_square</span>
                                                </a>

                                                <a href="#"
                                                    onclick="event.preventDefault(); confirmDelete({{ $package->id }});">
    ...
    table.on('draw', function() {
        $('select[name="status"]').off('change').on('change', function() {
            ...
            $.ajax({
                url: '/packages/' + membershipId + '/status',
                type: 'POST',
                data: {
                    _token: token,
                    status: status
