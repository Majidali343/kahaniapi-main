                                    ...
                                    @foreach ($orders as $order)
                                        <tr
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                @if ($order->admin_paid == 'false')
                                                    @if ($order->organization_request == 'true')
                                                        Organization requested for payment
                                                    @else
                                                    Not yet requested
                                                    @endif
                                                @else
                                            ...
                                            <td class="px-6 py-4">
                                                @php
                                                  $organizationstake =  $order->organization_stake . '% =  ' . ($order->price * $order->organization_stake) / 100 . ' PKR'
                                                @endphp
                                               {{ $organizationstake }}
                                            </td>
                                            <td class="px-6 py-4">
                                                @php
                                                  $discountpercentage = $order->discount_percentage . '% =  ' . ($order->price * $order->discount_percentage) / 100 . ' PKR'
                                                @endphp
                                                {{ $discountpercentage }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $order->price - ($order->price * $order->discount_percentage) / 100 -  ($order->price * $order->organization_stake) / 100 }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $order->coupon }}
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                @if ($order->admin_paid == 'false')
                                                    <form id="updateStatusForm"
                                                        data-membership-id="{{ $order->user_id }}">
                                                        @csrf
                                                        <select name="status" required>
                                                            <option value="true"
                                                                {{ $order->admin_paid == 'true' ? 'selected' : '' }}>
                                                                PAY
                                                            </option>
                                                            <option value="false"
                                                                {{ $order->admin_paid == 'false' ? 'selected' : '' }}>
                                                                UNPAID</option>
        ...
        $(document).ready(function() {
            ...
            table.on('draw', function() {
                $('select[name="status"]').off('change').on('change', function() {
                    ...
                    $.ajax({
                        url: '/payment/' + membershipId + '/status',
                        type: 'POST',
                        data: {
                            _token: token,
                            status: status
