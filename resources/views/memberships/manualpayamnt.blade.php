                                ...
                                @if (isset($manualpayments))
                                    ...
                                    @foreach ($manualpayments as $manualpayment)
                                        <tr
                                            ...
                                            <td class="px-6 py-4">
                                                {{ $manualpayment->paidamount }} PKR
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $manualpayment->type }}
                                            </td>
                                            <td class="px-6 py-4">
                                                @if ($manualpayment->payment_image)
                                                
                                                    <a href="{{ asset('/storage') . '/' . $manualpayment->payment_image }}"
                                                        target="_blank">
                                                        <img height="100px" width="120px"
                                                            src="{{ asset('/storage') . '/' . $manualpayment->payment_image }}"
                                                            alt="{{ $manualpayment->name }}">
                                                    </a>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ date('d-m-Y', strtotime($manualpayment->updated_at)) }}
                                            </td>
                                            <td class="px-6 py-4">
                                                ...
                                                @else
                                                    <form id="updateStatusForm"
                                                        data-membership-id="{{ $manualpayment->user_id }}">
                                                        @csrf
                                                        <select name="status" required>
                                                            <option value="approved"
                                                                {{ $manualpayment->status == 'approved' ? 'selected' : '' }}>
                                                                Approved
                                                            </option>
                                                            <option value="pending"
                                                                {{ $manualpayment->status == 'pending' ? 'selected' : '' }}>
                                                                pending</option>
                                                        </select>
                                                    </form>
                                                    <div id="statusMessage-{{ $manualpayment->user_id }}"></div>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
