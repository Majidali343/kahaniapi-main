                    ...
                    <form action="{{ route('package.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="validity" name="validity"
                                ...
                                <option value="lifetime" {{ old('validity') == 'lifetime' ? 'selected' : '' }}>Lifetime</option>
                                <option value="yearly" {{ old('validity') == 'yearly' ? 'selected' : '' }}>Yearly</option>
                                <option value="quarter" {{ old('validity') == 'quarter' ? 'selected' : '' }}>Quarter</option>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="permissions" name="Permissions"
                                ...
                                <option value="1" {{ old('Permissions') == '1' ? 'selected' : '' }}>Reading</option>
                                <option value="2" {{ old('Permissions') == '2' ? 'selected' : '' }}>Reading + Listening</option>
                                <option value="3" {{ old('Permissions') == '3' ? 'selected' : '' }}>Reading + Listening + Watching</option>
                            </select>
                            @error('Permissions')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <!-- Hidden input to store the Permissions value when Lifetime is selected -->
                            <input type="hidden" id="hiddenPermissions" name="Permissions" value="">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="title" min="0" name="price"
                                value="{{ old('price') }}" placeholder="Enter Amount"
    ...
    <script>
        document.getElementById('validity').addEventListener('change', function () {
            ...
            if (validityValue === 'lifetime') {
                ...
                hiddenPermissionsInput.value = '2'; // Set the hidden input to match the disabled select
                hiddenPermissionsInput.disabled = false; // Enable the hidden input to submit its value
            } else {
                permissionsSelect.removeAttribute('disabled');
                permissionsSelect.value = ''; // Reset permissions dropdown
                hiddenPermissionsInput.value = ''; // Clear the hidden input when not needed
                hiddenPermissionsInput.disabled = true; // Disable hidden input to prevent submission
            }
        });
    
        // Trigger change event on page load to apply default settings if lifetime is selected
        document.getElementById('validity').dispatchEvent(new Event('change'));
    </script>
