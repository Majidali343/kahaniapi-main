                                ...
                                @if (isset($orders))
                                    ...
                                    @foreach ($orders as $order)
                                        <tr
                                            ...
                                            <td class="px-6 py-4">
                                                {{ $order->organization_stake . '% =  ' . ($order->price * $order->organization_stake) / 100 . ' PKR' }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ $order->coupon }}
                                            </td>
                                                ...
                                                @if ($order->admin_paid == 'false')
                                                    ...
                                                    <form id="updateStatusForm"
                                                        ...
                                                        <select name="status" required>
                                                            <option value="true"
                                                                {{ $order->organization_request == 'true' ? 'selected' : '' }}>
                                                                REQUEST
                                                            </option>
                                                            <option value="false"
                                                                {{ $order->organization_request == 'false' ? 'selected' : '' }}>
                                                                UNPAID</option>
        ...
        $(document).ready(function() {
            ...
            table.on('draw', function() {
                $('select[name="status"]').off('change').on('change', function() {
                    ...
                    $.ajax({
                        url: '/orgpayment/' + membershipId + '/status',
                        type: 'POST',
                        data: {
                            _token: token,
                            status: status
